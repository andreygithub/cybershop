Cybershop.grid.FilterTable = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,baseParams: { action: 'mgr/categorys/categorysfiltertable/getList'}
        ,save_action: 'mgr/categorys/categorysfiltertable/updateFromGrid' 
        ,fields: ['id','category','filter','filter_name']
        ,paging: true
        ,pageSize: 5
        ,autosave: true
        ,remoteSort: true
...
Ext.extend(Cybershop.grid.FilterTable,MODx.grid.Grid,{
        setFilter: function() {
            this.getStore().baseParams.category = this.config.filterId;
        ...
        ,updateElement: function(btn,e) {
            if (!this.updateWindow) {
                this.updateWindow = MODx.load({
                    ...
                    ,baseParams: {
                        action: 'mgr/categorys/categorysfiltertable/update'
                    }
                    ,listeners: {
        ...
        ,removeElement: function() {
            ...
            ,url: this.config.url
            ,params: {
                action: 'mgr/categorys/categorysfiltertable/remove'
...
Cybershop.window.FilterTableElement = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,url: Cybershop.config.connectorUrl
        ,baseParams: {
            action: 'mgr/categorys/categorysfiltertable/create'
        }
        ,width: 700  
        ,minWidth:700
        ,fields:
